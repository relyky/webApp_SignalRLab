...
app.controller('mainCtrl', ['$scope', '$log', 'appValue', 'apiSvc', '$filter', function ($scope, $log, appValue, apiSvc, $filter) {
    ...
    var hubProxy = $.connection.monitorHub;
    ...
    //# method
    $scope.enableMonitorHub = function () {
        // open monitorHub
        if ($.connection.hub.state == 1) {
            hubProxy.server.open().done(function () {
                hubProxy.server.getState().done(function (rsp) {
                    _hobMonitorState = rsp;
                    $scope.$apply(); // refresh UI
                });
            });
        }
        else {
            alert("MonitorHub 尚未連線！");
        };
    };

    //# method
    $scope.disableMonitorHub = function () {
        // close monitorHub
        if ($.connection.hub.state == 1) {
            hubProxy.server.close().done(function () {
                hubProxy.server.getState().done(function (rsp) {
                    _hobMonitorState = rsp;
                    $scope.$apply(); // refresh UI
                });
    ...
    $scope.startMonitorHub = function (enable) {
        ...
        $.connection.hub.start()
            .done(function () {
                ...
                if (enable == true){
                    $scope.enableMonitorHub();
                }
            })
            .fail(function () {
                var msg = 'To connect MonitorHub fail!';
                $log.log(msg);
                $scope.$apply(); // refresh UI
                alert(msg);
            });
    ...
    $scope.stopMonitorHub = function () {
        $.connection.hub.stop()
            .done(function () {
                var msg = 'Stop connection. connection.hub.id =' + $.connection.hub.id;
                $log.log(msg);
                $scope.$apply(); // refresh UI
                alert(msg);
            })
            .fail(function () {
                var msg = 'To stop MonitorHub fail!';
                $log.log(msg);
                $scope.$apply(); // refresh UI
                alert(msg);
            });
    ...
    var _hobMonitorState = 'Unknow';
    Object.defineProperty($scope, 'hobMonitorState', {
        get: function () {
            if ($.connection.hub.state == 1)
            {
                hubProxy.server.getState().done(function (rsp) {
            ...
            {
                _hobMonitorState = 'Unknow';
            }

            return _hobMonitorState;
        }
    });
